...
if __name__ == "__main__":
    ...
    while is_trade_remaining(tam.portfolio_trades) and iteration < max_iterations:
        ...
        for symbol, qty in sorted_trades:
            ...
            if direction == "buy":
                account_id = select_account_for_buy_trade(symbol, qty_remaining, list(tam.accounts.values()), prices, tam.cash_matrix)
            else:
                account_id = select_account_for_sell_trade(symbol, qty_remaining, list(tam.accounts.values()))
    ...
    if iteration >= max_iterations:
        print("⚠️ Maximum iterations reached. Possible infinite loop or unresolvable trade drift.")

    print("\n=== Final Account States ===")
    for acc in accounts:
        print(f"{acc.account_number}: positions={acc.positions}, cash={tam.cash_matrix[acc.account_number]:.2f}")
