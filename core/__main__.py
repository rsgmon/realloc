...
def main():
    ...
    parser.add_argument("--rebalance", help="Path to input JSON with accounts, model, and prices")
    ...
    if args.rebalance:
        ...
        total_value = total_cash + sum(qty * prices[sym] for sym, qty in combined.items())
        target_shares = {sym: (weight * total_value) / prices[sym] for sym, weight in model.normalize().items()}
