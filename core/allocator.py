...
class PortfolioAllocator:
    def __init__(self, accounts: List[Account], model: PortfolioModel, prices: Dict[str, float]):
    ...
    def compute_portfolio_trades(self):
        ...
        total_value = total_cash + sum(qty * self.prices[sym] for sym, qty in combined.items())
    ...
    def rebalance(self) -> Dict[str, int]:
        ...
        self.tam = TradeAccountMatrix(self.accounts, self.prices, self._portfolio_trades)
    ...
    def get_account_positions(self) -> Dict[str, Dict[str, float]]:
        return {
            acc.account_number: acc.positions
            for acc in self.accounts
        }
